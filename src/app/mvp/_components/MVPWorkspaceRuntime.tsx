...
const saveStateLabel = (
    ...
    if (state === "saving") return "Saving";
        if (state === "recovered") {
            ...
            if (workspaceOrigin === "demo") return "Sample shell loaded";
            return "Recovered";
        }
...
const MVPWorkspaceClarityHeader = React.memo(function MVPWorkspaceClarityHeader({
    ...
    const returnLabel = returnToLaunchpadHref?.includes("/app/worlds/") ? "Return to project record" : "Return to launchpad";
    return (
...
const MVPWorkspaceFrame = React.memo(function MVPWorkspaceFrame({
    ...
    const previewWorkspaceNavigation =
        clarityMode && routeVariant === "launchpad"
            ? {
                  eyebrow: workspaceSession.launchProjectId ? "Project launch" : workspaceSession.workspaceOrigin === "demo" ? "Sample shell" : "World entry",
                  title: workspaceSession.launchProjectId
                      ? workspaceSession.hasSavedVersion
                          ? "Project world record"
                          : "Project launch"
                      : workspaceSession.workspaceOrigin === "demo"
                        ? "Sample shell"
                        : "Intake workspace",
                  note: workspaceSession.launchProjectId
                      ? workspaceSession.hasSavedVersion
                          ? "Return to the project world record."
                          : "Return to the project record and source paths."
                      : workspaceSession.workspaceOrigin === "demo"
                        ? "Return to the world entry without losing this sample shell."
                        : "Return to the world entry without losing this intake workspace.",
                  backLabel: workspaceSession.launchProjectId ? "Return to project" : "Return to launchpad",
